...
function importJSON(e, onImport) {
  ...
  reader.onload = ev => {
    try {
      ...
      if (Array.isArray(data)) { onImport(data); alert(`${data.length}件のデータを復元しました`) }
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2 no-print">
        ...
        <div className="flex gap-2">
          ...
          <label className="flex-1 py-2 rounded-lg border border-slate-300 text-slate-600 text-xs font-bold hover:bg-slate-50 text-center cursor-pointer">
            📤 復元（JSON）
            <input type="file" accept=".json" className="hidden" onChange={e => onImport && importJSON(e, onImport)} />
          </label>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4 text-sm text-slate-700 leading-relaxed">
      ...
      {[
        ...
        ['第2条（個人情報の取り扱い）', '本アプリに入力されたデータは、利用者の端末内（ブラウザのlocalStorage）にのみ保存され、外部サーバーへの送信は一切行いません。ただし、localStorageはブラウザの開発者ツールで閲覧可能であり、共有端末での使用には十分ご注意ください。'],
